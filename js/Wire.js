...
Game.getWire = function (myX, myY, myType) {
    'use strict';
...
Game.Wire = function (myX, myY, myType, tRotation, tCanRotate) {
	var my = Game.getWire(myX, myY, myType),
		wallArray = {
			"corner" : [1,1,0,0],
			"straight" : [1,0,1,0],
			"plug" : [1,0,1,0],
	...
	my.addEventListener("mousedown", function () {
		my.rotateOnTap();
		Game.checkConnection();
    });
	
	var getOutgoingWires = function(){
		var outWireDirections = [];
		if(wallArray[0] === 1){
			outWireDirections.push({'X':0, 'Y':-1});
		}
		if(wallArray[1] === 1){
			outWireDirections.push({'X':1, 'Y':0});
		}
		if(wallArray[2] === 1){
			outWireDirections.push({'X':0, 'Y':1});
		}
	...
	my.lightUp = function(){
		...
		for(outIndex in outWireDirections){
			...
			if(isInside){
				Game.wires.get(testX,testY).tryToLightUp(dirX, dirY);
			}
		}
	};
